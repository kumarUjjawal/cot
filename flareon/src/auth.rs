...
pub trait User {
    ...
    /// the session (e.g. password change).
    ///
...
type SessionAuthHmac = Hmac<Sha3_256>;
...
impl AuthRequestExt for Request {
    async fn user(&mut self) -> Result<&dyn User> {
        if self.extensions().get::<UserExtension>().is_none() {
            let user_id = self.session().get::<UserId>(USER_ID_SESSION_KEY).await?;
            if let Some(user_id) = user_id {
                ...
                if session_auth_hash_valid(&*user, self).await? {
...
async fn session_auth_hash_valid(
    ...
    let user_hash = user.session_auth_hash(config.secret_key());
        ...
        if user_hash == stored_hash {
            ...
            for fallback_key in config.fallback_secret_keys() {
                ...
                if user_hash_fallback == stored_hash {
                    request
                        .session_mut()
                        .insert(SESSION_HASH_SESSION_KEY, user_hash_fallback.as_bytes())
                        .await?;
...
mod tests {
    ...
    struct MockAuthBackend<T> {
        user: T,
    }

    #[async_trait]
    impl<T: Fn() -> MockUser + Send + Sync + 'static> AuthBackend for MockAuthBackend<T> {
        async fn authenticate(
            &self,
            _request: &Request,
            _credentials: &(dyn Any + Send + Sync),
        ) -> Result<Option<Box<dyn User + Send + Sync>>> {
            Ok(Some(Box::new((self.user)())))
        }
    ...
    fn create_request<T: Fn() -> MockUser + Send + Sync + 'static>(return_user: T) -> Request {
        ...
        TestRequestBuilder::get("/")
            ...
            .with_config(
                ProjectConfig::builder()
                    .auth_backend(
                        Arc::new(MockAuthBackend { user: return_user }) as Arc<dyn AuthBackend>
                    )
                    .clone()
                    .build(),
            )
    ...
    async fn test_login_logout() {
        let mut request = create_request(MockUser::new);
    ...
    #[tokio::test]
    async fn test_logout_on_session_hash_change() {
        ...
        let mut request = create_request(create_user.clone());
